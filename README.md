...
GitHub Releases provide **amd64** `.deb` packages for Debian **12 / 13** and
Ubuntu **22.04 / 24.04 / 26.04 LTS**. Each package is built and installation-tested
inside its target distribution, with runtime library dependencies generated by
Debian's packaging tools. Choose the file matching your distribution; for example:

```sh
sudo apt install ./ubgp_0.2.0-1.debian12_amd64.deb
sudoedit /etc/ubgp.toml
sudo /usr/sbin/ubgp --check-config
sudo systemctl enable --now ubgp
```

Edit the example addresses, ASNs, interface and export ACLs before starting the
service. The package installs `/usr/sbin/ubgp`, a systemd unit, and
`/etc/ubgp.toml` as a dpkg-managed configuration file. It preserves local
configuration on upgrades. Installation does not enable or start the service;
upgrades do not restart it. After upgrading a running deployment, use
`sudo systemctl restart ubgp` when ready. The optional receive-buffer tuning
file is in `/usr/share/doc/ubgp/examples/90-ubgp.conf` and is not applied
automatically.

### Generate packages in GitHub

For packages without a release: open **Actions → Debian and Ubuntu packages →
Run workflow**, choose the branch or tag, and run it. Download the
**ubgp-debs-amd64** artifact when all jobs finish. It contains all five `.deb`
files and `SHA256SUMS`.

To publish a release:

1. Update `version` in `Cargo.toml`, refresh `Cargo.lock` with `cargo check`, and
   commit and push both files along with any release changes.
2. Open **Releases → Draft a new release**, create a tag matching the Cargo
   version (for example `v0.2.0`), and select the commit to release.
3. Click **Publish release**. The workflow builds and tests all five packages,
   then attaches them and `SHA256SUMS` to that release. Prereleases work too;
   their tag must also match the Cargo version exactly.

The tag must be `v` followed by the version in `Cargo.toml`; a mismatch fails
the build. Assets appear after the workflow completes. Re-running the workflow
replaces its existing package assets. A manual workflow run only creates Actions
artifacts, even when a tag is selected. The workflow uses GitHub's built-in token;
no release secret is needed.

After downloading all release assets, verify them with `sha256sum -c SHA256SUMS`.

### Generate packages locally

Requires Git and Docker with Buildx. Run from a repository checkout:
...
# Or just one:
./packaging/build-debs.sh ubuntu24.04
```

Packages and checksums appear under `dist/<distribution>/`. Supported target
names are `debian12`, `debian13`, `ubuntu22.04`, `ubuntu24.04`, and `ubuntu26.04`.
The containers use pinned Rust 1.97.1 and `Cargo.lock`; no host Rust toolchain is
needed. Builds target `linux/amd64`; non-amd64 hosts need Docker's amd64 emulation.
Each build runs unit tests and validates the example configuration, then checks
package installation, CLI startup, configuration preservation, removal and purge
in a clean container. These checks do not boot systemd or exercise live BGP
sessions; the network integration suite below covers protocol behavior.
